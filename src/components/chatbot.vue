<template>
    <div class="flex flex-col items-end gap-5 absolute right-5 max-[640px]:top-20 min-[640px]:bottom-5">
        <div
            class="h-[75vh] w-[50vw] max-w-screen-md min-w-[30rem] max-[640px]:hidden bg-darkLight/50 backdrop-blur-2xl rounded-md shadow-2xl p-5 flex flex-col gap-5 justify-center"
            v-if="showChatbot"
        >
            <div class="flex flex-row items-start justify-between">
                <button
                    class="aspect-square bg-dark text-lightBlue rounded-md flex items-center justify-center p-1 hover:bg-lightBlue active:bg-lightBlue hover:text-dark active:text-dark"
                    @click="resetChat"
                >
                    <Icon name="bi:trash3-fill" class="max-[480px]:text-2xl min-[480px]:text-3xl" />
                </button>
                ...
                <button
                    class="aspect-square bg-dark text-red rounded-md flex items-center justify-center p-1 hover:bg-red active:bg-red hover:text-dark active:text-dark"
                    @click="toggleChatbotView"
                >
                    <Icon name="ic:round-close" class="max-[480px]:text-2xl min-[480px]:text-3xl" />
                </button>
            </div>
            ...
            <div class="w-full flex flex-row gap-5 items-center">
                ...
                >
                    <Icon name="ic:round-send" class="max-[480px]:text-2xl min-[480px]:text-3xl" />
                </button>
            </div>
        </div>
        ...
        <button
            ...
            @click="toggleChatbotView"
        >
            <Icon name="mage:robot-happy-fill" class="max-[480px]:text-2xl max-[640px]:text-3xl min-[640px]:text-5xl" />
        </button>
    </div>

    <div
        class="h-full w-full absolute bottom-0 top-0 left-0 right-0 min-[640px]:hidden bg-darkLight/50 backdrop-blur-2xl rounded-md shadow-2xl p-5 flex flex-col gap-5 justify-start z-30"
        v-if="showChatbot"
    >
        <div class="flex flex-row items-start justify-between gap-2.5">
            ...
            <span class="font-bold text-center max-[480px]:text-md max-[640px]:text-xl min-[640px]:text-2xl">
                {{
                    language === Language.FR
                        ? "Une question à propos de Mathieu ? "
                        : "You have a question about Mathieu ? "
